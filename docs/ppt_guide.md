...
| **Non-differentiable Barrier** | 모델 출력(layout) → Qiskit transpile → PST 사이에 미분이 불가능한 구간. transpile이 미분 불가능하므로 PST에서 모델로 직접 gradient를 전달할 수 없음. Surrogate loss 설계의 근본 원인. |
...
**핵심 메시지:** Non-differentiable barrier 때문에 PST를 직접 최적화할 수 없으므로, 미분 가능한 surrogate loss로 학습.

```
Unsupervised Surrogate Training (전체 데이터 사용)
  L_adj + β·L_hop + α·L_node → noise-aware 최적화
```

---

### Slide 14: Unsupervised Surrogate Training (상세)
...
**핵심 메시지:** 5개 데이터셋, 총 6,887 회로. Labeled 4,269개 포함, 전체 unlabeled로 surrogate loss 학습.

**내용 (표):**

| Dataset | 회로 수 | Label 유무 |
|---------|:-------:|:----------:|
| QUEKO | 900 (540 labeled) | O (540) |
| MLQD | 4,443 (3,729 labeled) | O (3,729) |
| MQT Bench | 1,219 | X |
| QASMBench | 94 | X |
| RevLib | 231 | X |
...
**Speaker Notes:**
5개 공개 데이터셋에서 총 6,887개 양자 회로를 사용합니다. Label 유무와 관계없이 전체 데이터를 unsupervised surrogate loss로 학습합니다.
...
- 학습 루프: surrogate losses, τ annealing, early stopping
