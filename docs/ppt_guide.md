...
| **Floyd-Warshall** | 그래프의 모든 노드 쌍 간 최단 경로를 계산하는 알고리즘. 하드웨어 그래프에서 error-weighted 최단 경로를 사전 계산하는 데 사용. |
...
Stage 2: Unsupervised Surrogate Fine-tuning (전체 데이터 사용)
  L_surr + α·L_node + λ·L_sep → noise-aware 최적화
```
...
- Stage 2의 surrogate loss만으로는 충분하지 않은 3가지 이유:
  ...
  3. **Trivial solution 위험:** L_surr은 "자주 상호작용하는 큐빗을 가까이 배치하라"만 지시. 의미 있는 사전 지식 없이는 의미 없는 local minimum에 수렴할 수 있음
...
**이 슬라이드의 목적:** Stage 2의 surrogate loss 3가지를 상세 설명.

**핵심 메시지:** Label 없이도 "좋은 매핑"의 신호를 제공하는 미분 가능한 surrogate loss 설계. 특히 noise를 고려.

**내용:**

**데이터:** 6,887 전체 회로 (label 불필요 → labeled + unlabeled 모두 사용)

**Total Loss = L_surr + α · L_node + λ · L_sep**

**L_surr (Error-Aware Edge Quality Loss) — 핵심 loss:**
- 목적: 논리적으로 자주 상호작용하는 큐빗 쌍을 물리적으로 "가까운 + 저오류 경로"에 매핑
- 작동 방식:
  - 하드웨어 그래프에서 모든 물리 큐빗 쌍 간 error-weighted 최단 경로를 사전 계산 (Floyd-Warshall 알고리즘)
  - 경로의 "거리" = cx_error(2-qubit gate 오류율)의 가중합
  - 따라서 오류율이 낮은 경로로 연결된 물리 큐빗 쌍이 "가까운" 것으로 취급
  - 논리 회로에서 상호작용이 많은 큐빗 쌍일수록 물리적으로 가까운 위치에 매핑되도록 유도
- 단순한 hop distance가 아닌 error-weighted distance를 사용 → noise-aware 매핑

**L_node (NISQ Node Quality Loss):**
- 목적: "바쁜" 논리 큐빗을 "고품질" 물리 큐빗에 매핑
- 작동 방식:
  - 각 물리 큐빗의 품질 점수를 learnable weighted combination으로 계산:
    q_score = w1·T1 + w2·T2 + w3·frequency + w4·readout_error + w5·single_qubit_error
  - 가중치 w1~w5는 모델이 학습
  - gate가 많은 논리 큐빗이 q_score가 높은 물리 큐빗에 배치되도록 유도
- 부수 효과: 학습 후 w1~w5 분석 → 어떤 noise factor가 PST에 가장 영향이 큰지 insight 제공 (해석 가능성)

**L_sep (Separation Loss):**
- Multi-programming(여러 회로 동시 실행)에서 서로 다른 회로의 큐빗이 물리적으로 떨어지도록 유도
- 현재 단일 회로에만 집중하므로 이 loss는 자동으로 0 (향후 확장용)

**다이어그램 제안:**
- 3개 loss를 시각적으로 분리하여 각각의 역할 도식화
  - L_surr: 회로 그래프의 엣지 → 하드웨어 그래프의 error-weighted path 대응 그림
  - L_node: 논리 큐빗(바쁜)와 물리 큐빗(고품질) 간의 매칭 그림
  - L_sep: (회색/투명 처리, "향후 확장" 표시)

**Speaker Notes:**
Stage 2에서는 label 없이도 계산 가능한 3가지 surrogate loss를 사용합니다. 핵심인 L_surr은 회로에서 자주 상호작용하는 큐빗 쌍을 하드웨어에서 오류율이 낮은 경로로 연결된 위치에 배치하도록 유도합니다. 단순한 홉 거리가 아닌 error-weighted 거리를 사용하여 noise를 고려합니다. L_node는 바쁜 논리 큐빗을 고품질 물리 큐빗에 배치하도록 하는데, 품질 점수의 가중치를 모델이 학습합니다. 학습 후 이 가중치를 분석하면 어떤 noise factor가 PST에 가장 중요한지 알 수 있어 해석 가능성도 제공합니다.
...
- Ablation study: Cross-Attention 유무, L_node/L_sep 기여도, τ annealing 전략 등
- 논문 작성
