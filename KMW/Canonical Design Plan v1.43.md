...
1. explicit implementation decisions locked for **Canonical Design Plan v1.42**,
...
The canonical-hardware v1.42 path is now:
...
## 3. Full Base Implementation Specification (Canonical v1.41)
## 3. Full Base Implementation Specification (Canonical v1.41)

# Canonical Design Plan v1.42 — Implementation Specification
...
- verify the canonical-to-native decode path works.
...
- the proxy evaluation outputs, and
- the routed downstream real metrics computed after transpilation/routing on the final native-ID mapping.

For research reporting and experiment validity, the routed downstream real metrics are **mandatory outputs**, not optional extras.

The routed evaluation section must write, at minimum, the following per-circuit fields and corresponding dataset-level summary aggregates:
...
If the implementation also reports a depth ratio, it may additionally include:

- `depth_increase_ratio`

Proxy metrics remain useful for diagnosis, but they do **not** replace the mandatory routed metrics above.
...
Then sample a circuit uniformly from the chosen source bucket.

This is a **required implementation rule**, not a deferred enhancement.

Implementation rule:
- the train dataset must preserve source identity for each row
- the training loader or sampler must choose a source first, then choose a sample uniformly within that source
- do **not** rely on simple concatenation plus shuffle for the official main run
- validation and test loaders may remain plain deterministic traversals
...
1. load batch through the source-balanced training sampler
2. extract or load canonicalized tensors
3. forward mapper to obtain `S_can`
4. deterministically decode `S_can` to `S_nat`
5. slice to active logical rows to obtain `S_nat_act`
6. compute `P_nat_act` from `S_nat_act`
7. compute native-frame-after-decode v1.4.1 loss with explicit `lambda_route`
8. log the required loss decomposition
9. backprop mapper parameters
10. update optimizer
...
9. source-balanced training sampler
10. trainer with mandatory loss-decomposition logging
11. evaluation
12. routed final evaluation hookup

Reason:

- canonical preprocessing must be trustworthy before mapper training has any meaning,
- deterministic decode must be correct before assignment/loss semantics are trustworthy,
- loss implementation must match Option A frame semantics before comparing against learned-reindexer v1.4.
...
- main-run training uses source-balanced sampling across `queko`, `mlqd`, and `mqt_bench`
...
- native-frame assignment uses the decoded logits consistently
- padded dummy rows do not participate in either Sinkhorn or Hungarian
...
- the loss decomposition (`L_1Q`, `L_RO`, `L_2Q`, `L_native`, `L_route`, `L_task`, `route_to_native_ratio`) is logged during training and validation
...
The canonical-hardware v1.42 branch keeps the streamlined v1.4 mapper architecture — circuit matrix as the only spatial U-Net input, hardware-only tokens as cross-attention conditioning, Sinkhorn for training, Hungarian for inference, and the v1.4.1 execution-surrogate loss family — but removes the learned reindexer entirely. Instead, the hardware is canonicalized once using a deterministic permutation computed from the backend topology and costs, while the circuit remains in original logical order. The mapper therefore predicts into canonical hardware slots, those logits are deterministically decoded back to native hardware order, assignment is performed only over active logical rows, the task loss is evaluated in the native frame after decode with an explicit `lambda_route` reweighting of routing pressure, and the main training run uses source-balanced sampling across the three train-side sources. The resulting native-ID mapping flows directly into downstream routed evaluation. This revision exists to isolate whether learned reindexing was the dominant cause of the performance drop while also correcting three residual performance risks: train-distribution skew, dummy-row assignment interference, and underweighted routing pressure.
