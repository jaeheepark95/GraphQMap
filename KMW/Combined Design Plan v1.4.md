> **Update note (2026-03-30, v1.4.1):** This document is amended again to integrate the
> loss-plan v1.4.1 replacement. Where any older section in this file still refers to
> `L_PST_total`, `L_swap`, `L_depth`, or the old lambda-weighted task loss, the new
> v1.4.1 execution-surrogate loss takes precedence. The new loss uses logical gate-count
> weighting (`n1Q`, `nmeas`), reliability-weighted route hazard (`Croute`), and interprets
> the final execution quantity as a surrogate score rather than literal PST.

# Combined Design Plan v1.31

## 0. Document status

This document supersedes **Combined Design Plan v1.3** where the two documents conflict.
It is not a fresh rewrite of every section. Instead, it preserves the v1.3 baseline and
adds a stabilization-focused amendment package covering the updates agreed after v1.3:

- **A. Full staged reindexer schedule enforcement**
- **B. Gentler stage-2 sharpening**
- **C. Explicit reindexer diagnostics**
- **D. `R_H = I` hardware-freeze ablation**
- **E. Optional deterministic logical canonical-teacher pretraining**

---

## 0.1 Update log (v1.31)

- revised staged defaults to use the gentler reindexer schedule discussed after v1.3
- formalized stage-2 `beta_cons` activation as part of the planned schedule
- added trainer-side diagnostics for `R_L / R_H` sharpness and entropy
- added a clean `freeze_hardware_reindex` ablation mode where `R_H = I`
- added an optional pretraining phase for `R_L` using a deterministic internal logical canonical teacher
- kept the main pipeline unchanged at a high level:
  `native -> learned soft reindexing -> mapper -> decode -> native-frame loss`

---

## 1. Revised staged-default schedule (A + B)

### 1.1 Default stage lengths

For the stabilized default curriculum, use:

- pretrain: `0` epochs by default (optional extension; see Section 4)
- warmup: `10` epochs
- stage 1: `25` epochs
- stage 2: `25` epochs

Total default training horizon for the stabilized curriculum:

- **60 epochs**

### 1.2 Default `tau_r` schedule
...
- `tau_r_mid   = 0.70`
- `tau_r_end   = 0.25`
- interpolation schedule: cosine by default, linear allowed as an ablation

Interpretation:

- warmup uses the identity reindex path and therefore does not depend on learned `R_L / R_H`
- stage 1 anneals from `1.0 -> 0.70`
- stage 2 anneals from `0.70 -> 0.25`

This is intentionally gentler than the earlier `1.0 -> 0.60 -> 0.15` plan because the
empirical instability clustered near the stage-2 transition.

### 1.3 Default auxiliary weights
...
- stage 1 remains locality-only
- stage 2 turns on consistency anchoring exactly as intended in the staged plan

---

## 2. Reindexer diagnostics package (C)

The trainer must log reindexer diagnostics both:

1. **per step** into `train_metrics.jsonl`
2. **per epoch** through the trainer’s aggregated epoch summary

### 2.1 Required diagnostics

For each training step, log:

- `RL_entropy_mean`
- `RH_entropy_mean`
- `RL_rowmax_frac_ge_0.5`
- `RL_rowmax_frac_ge_0.7`
- `RL_rowmax_frac_ge_0.9`
- `RH_rowmax_frac_ge_0.5`
- `RH_rowmax_frac_ge_0.7`
- `RH_rowmax_frac_ge_0.9`
- `passB_L_loc`
- `passB_L_cons`

### 2.2 Semantics

- entropy is mean row entropy of the soft permutation matrix, normalized by `log(N)`
- row-max sharpness fraction is the fraction of rows whose maximum entry exceeds the threshold
- these are scalar trainer diagnostics only; they do not change the optimization objective directly

### 2.3 Stop/go interpretation

Desired trends:

- `RL_entropy_mean` should decrease over training
- `RH_entropy_mean` should not remain suspiciously constant unless the run is intentionally in the `R_H = I` ablation mode
- row-max sharpness fractions should rise, especially on the logical side

---

## 3. Hardware-freeze ablation (D)

### 3.1 Purpose

This ablation exists to answer a specific question:

> Is hardware-side learned reindexing actually helping, or is it adding unnecessary freedom
> while the logical-side reindexer is the dominant source of useful structure?

### 3.2 Definition

When `freeze_hardware_reindex = True`:

- `R_H` is forced to the identity matrix for the whole run
- only `R_L` remains learnable
- the hardware reindex branch does not contribute learnable behavior to the forward pass
- inference/evaluation must use the same identity-`R_H` rule for a clean ablation

### 3.3 Scope

This is an **ablation mode**, not the new global default.

### 3.4 Consistency-loss behavior in this mode

Because `R_H = I`, hardware-side permutation consistency would otherwise become an impossible or
meaningless target. Therefore, under this ablation:

- consistency loss is interpreted logically
- the logical reordered outputs remain consistency-constrained
- hardware-side consistency terms are disabled in the consistency computation

This keeps `beta_cons` usable during the `R_H = I` ablation without penalizing the model for not
learning a hardware permutation that was explicitly frozen away.

---

## 4. Optional deterministic logical canonical-teacher pretraining (E)

### 4.1 Principle

This pretraining phase is **not** a return to the old fixed canonical-ID main pipeline.
The main runtime path remains:

`native -> learned soft reindexing -> mapper -> decode -> native-frame loss`

The canonical teacher exists only as a trainer-side initialization aid for `R_L`.

### 4.2 Default recommendation

- disabled by default
- enabled only when the logical reindexer fails to sharpen adequately under the staged curriculum alone

### 4.3 Scope of pretraining

When enabled:

- pretrain **`R_L` only**
- force **`R_H = I`** during pretraining
- do **not** train the mapper during this phase
- after pretraining finishes, hand off to the normal staged curriculum

### 4.4 Teacher definition

The internal teacher is a deterministic logical canonical order built from the logical interaction matrix `A`
and logical occupancy mask `m`.

Recommended implementation:

1. identify active logical slots from `m`
2. choose a root using a deterministic root-score rule from `A`
3. expand with a BFS-style traversal whose neighbor order is determined by descending interaction strength
4. if the active logical graph is disconnected, restart from the strongest remaining unvisited active node
5. append inactive padded logical slots at the end in identity order

The resulting hard permutation matrix is used as a teacher target for `R_L`.

### 4.5 Pretraining loss

For the first implementation, use a simple stable objective:

- mean-squared error between the soft `R_L` and the hard logical teacher permutation

This keeps the pretraining phase easy to inspect and robust to implementation mistakes.
...
- staged curriculum scheduling
- canonical-pretrain phase
- `freeze_hardware_reindex` training-time semantics
- per-step and per-epoch diagnostics logging
- optimizer grouping and parameter-freeze policy

### 5.2 `model.py`

Owns:

- reindexer support for identity-`R_H` operation
- deterministic logical canonical-teacher utilities
- helper functions needed by trainer/evaluation for the ablations

### 5.3 `evaluate.py`

Owns:

- inference-time support for `freeze_hardware_reindex`
- preserving train/eval semantic consistency for the D ablation

### 5.4 `main.py`

Owns CLI/config exposure for:

- `freeze_hardware_reindex`
- `enable_canonical_pretrain`
- `pretrain_epochs`
- `pretrain_tau_r`
- `pretrain_teacher_loss_weight`

---

## 6. Recommended experimental order after v1.31

1. **A + B only**
   - gentler schedule, stage-2 consistency on
2. **A + B + C**
   - verify entropy and sharpness trends
3. **A + B + C + D**
   - clean `R_H = I` ablation
4. **A + B + C + E**
   - canonical-teacher initialization for `R_L`
5. **A + B + C + D + E**
   - strongest stabilization/ablation package when logical-side sharpening remains weak

---

## 7. Base document carried forward from v1.3

The remainder of this file preserves the v1.3 baseline text. Where any statement below conflicts
with Sections 0-6 above, the v1.31 amendment takes precedence.

---

# Combined Design Plan v1.2
...
### 6.4a Raw per-qubit error preservation

In addition to `c1_raw`, preserve the separated raw per-qubit error tensors:
...
These raw tensors are cached and returned for the loss/evaluation path only.

### 6.5a Raw per-edge error preservation
...
### 6.6a Raw distance preservation

In addition to normalized `D`, preserve raw shortest-path distance as:

- `D_raw`

The mapper and reindexer continue to use normalized `D`.
...
## 12. Task Loss (v1.4.1 replacement)

This revision replaces the older task-loss family

`L_task = lambda_P * L_PST_total + lambda_S * L_swap + lambda_D * L_depth`

with the v1.4.1 error-rate-style execution surrogate.

Important implementation note:

- the mapper and reindexer still consume the normalized model-side tensors `c1`, `c2`, and `D`
- the new loss additionally consumes raw backend-side error tensors and raw shortest-path distance
- the new loss also consumes two circuit-side logical count vectors `n1Q` and `nmeas`

### 12.1 Symbols and variables

Index sets:

- `u, v ∈ {0, 1, ..., 26}` are logical-qubit indices
- `i, j, a, b ∈ {0, 1, ..., 26}` are physical-qubit indices

Current tensors:

- `A[u, v]` : weighted logical interaction matrix
- `m[u]` : logical-valid mask
- `B[i, j]` : hardware adjacency matrix
- `P_map[u, i]` : Sinkhorn soft assignment from logical qubit `u` to physical qubit `i`

Extended logical-count tensors:

- `n1Q[u] ≥ 0` is the number of logical 1Q gates applied to logical qubit `u`
- `nmeas[u] ≥ 0` is the number of logical measurement operations applied to logical qubit `u`

Derived interaction tensor and normalization masses:

`Aoff[u, v] = A[u, v]` for `u != v`, else `0`

`mass1Q   = max(sum_u n1Q[u], 1e-6)`
`massmeas = max(sum_u nmeas[u], 1e-6)`
`mass2Q   = max(sum_{u,v} Aoff[u, v], 1e-6)`

Raw hardware error rates:

- `e1Q[j] ∈ [0,1)` is the raw 1Q gate error rate of physical qubit `j`
- `ero[j] ∈ [0,1)` is the raw readout error rate of physical qubit `j`
- `e2Q[i,j] ∈ [0,1)` is the raw native 2Q gate error rate on hardware edge `(i,j)` when `B[i,j] = 1`
- `D_raw[i,j]` is the raw shortest-path hop distance between physical qubits `i` and `j`

Single-use survival factors:
...
- `eps_surv > 0` is the numerical clamp used before any logarithm; default `1e-12`

Native hazard transforms:

- `q1Q[j] = -log(max(s1Q[j], eps_surv))`
- `qro[j] = -log(max(sro[j], eps_surv))`
- `q2Q[i,j] = -log(max(s2Q[i,j], eps_surv))`

### 12.2 Reliability-weighted routing quantities

Per-edge routing-step hazard:

`cstep[a,b] = 3 * q2Q[a,b] + 2 * (q1Q[a] + q1Q[b]) / 2`

This uses the first-order approximation “one SWAP ≈ three native 2Q gates” plus a small 1Q-overhead correction.

Adjacency-reaching path hazard:

For a shortest path `π = (v0=i, v1, ..., vK=j)` with `K = D_raw[i,j]`, define

`Hpath(π; i, j) = sum_{t=0}^{K-2} cstep[vt, vt+1]`

This sums exactly the first `D_raw[i,j] - 1` route steps needed to bring the endpoints into adjacency.

Reliability-weighted route-cost matrix:

`Croute[i,j] = 0` if `i = j` or `B[i,j] = 1`

Otherwise,

`Croute[i,j] = min_{π ∈ Πsp(i,j)} Hpath(π; i, j)`

where `Πsp(i,j)` is the set of all shortest hardware paths from `i` to `j`.

### 12.3 Soft survival and route-surrogate definitions

Soft 1Q survival:

`S1Q[u] = sum_j P_map[u,j] * s1Q[j]`

Soft readout survival:

`Sro[u] = sum_j P_map[u,j] * sro[j]`

Soft native 2Q survival:

`S2Q[u,v] = sum_{i,j} P_map[u,i] * P_map[v,j] * B[i,j] * s2Q[i,j]`

Soft routing hazard:

`Hroute[u,v] = sum_{i,j} P_map[u,i] * P_map[v,j] * Croute[i,j]`

Soft route survival surrogate:

`Sroute[u,v] = exp(-Hroute[u,v])`

### 12.4 Final loss definitions
