...
2. `Attn_Unet_27_SPEC.md` for the mapper backbone,
3. `Design Plan v1.1.1 — Clarification Patch.md`,
4. `Design Plan v1.1 - reformatted.md`,
5. `project overview.pdf`, `readme.md`, and `file_explanation_v1.pdf` for structure, intent, and non-conflicting context.
...
- smoke behavior is determined by config, not by special split names,
- use relative paths only,
- keep enough metadata for filtering and debugging, but not unnecessary bulk,
- split manifests are authoritative and should be reused once created.

Recommended per-row schema:

```json
{
  "id": "mqt_example_000123",
  "source": "mqt",
  "split": "train",
  "qasm_relpath": "data/circuits/mqt/.../example_000123.qasm",
  "k_logical": 12,
  "num_1q": 85,
  "num_2q": 47,
...
- Keep `KMW/data/circuits/` as the local raw-circuit area for this project.
- Right now, only the **MQT** dataset is downloaded.
...
Design Plan v1.1.1.md — implementation authority for the KMW method.
...
configs/base.yaml — shared defaults for paths, model size, loss weights, and
training settings.
...
configs/train_main.yaml — config for the main training run.

•

configs/eval.yaml — config for evaluation and test-time inference.
...
docs/training_protocol.md — rules for training stages, logging, and fail-fast
checks.
...
data/circuits/ — raw circuit datasets used by KMW.

•

data/circuits/mqt/ — local raw MQT dataset folder; currently the only downloaded
dataset.

•

data/manifests/ — train/val/test file lists used by loaders.

•

data/filtered/ — filtered indices such as K <= 27, valid parse, and non-degenerate
circuit lists.

•

data/cache/ — cached backend tensors, circuit features, and preprocessing
results.
...
src/kmw/data/ — dataset loading and split logic.
...
src/kmw/data/dataset.py — main dataset code, manifest loading, filtering hooks,
and dataloader-facing sample return logic.
...
src/kmw/preprocessing/pipeline.py — backend extraction, circuit featurization,
normalization, token construction, distance computation, and validation checks.
...
src/kmw/models/model.py — reindexer, hardware token encoder, U-Net mapper,
cross-attention, and assignment helpers.
...
src/kmw/losses/loss.py — task loss, PST-related proxy terms, swap/depth proxies,
and reindexer auxiliary losses.
...
src/kmw/training/trainer.py — two-pass training loop, optimizer setup,
checkpointing, logging, and fail-fast checks.
...
src/kmw/evaluation/evaluate.py — inference pipeline, hard reindexing, datasetlevel evaluation, and summary report generation.
...
src/kmw/cli/main.py — unified CLI entrypoint for data prep, training, and
evaluation commands.
...
scripts/build_manifests.py — builds train/val/test manifests from available circuits.
...
scripts/run_eval.sh — runs evaluation on the configured test set.
...
tests/unit/ — small isolated tests for preprocessing, model pieces, and losses.

•

tests/integration/ — multi-module tests such as dataset-to-forward-pass checks.
...
runs/ — temporary experiment outputs such as run folders and intermediate
results.

artifacts/
•

artifacts/ — saved checkpoints, logs, figures, and final reports.
...
pipeline.py should handle backend extraction, A/B/c1/c2/D construction,
normalization, and validators.

•

model.py should contain the reindexer, token encoder, mapper, attention, and
assignment helpers.
...
trainer.py should contain both passes of training, logging, fail-fast checks, and
checkpoint saving.

•

evaluate.py should contain inference, transfer evaluation, and report summaries.
...
Since only data/circuits/mqt/ currently exists, the first implementation target should be:
1. make the MQT smoke pipeline run,
2. verify preprocessing and training stability,
3. then expand manifests and configs when the other datasets are added.
...
- updated the plan filename to `Design Plan v1.1.1.md`
