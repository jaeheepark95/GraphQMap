...
# - Added reindexer support for freeze_hardware_reindex / identity R_H ablations.
# - Added deterministic logical canonical-teacher utilities for optional R_L pretraining.
# - Added identity-permutation helpers used by trainer/evaluation without changing
#   the main native -> learned reindexing -> mapper -> decode pipeline.
# =============================================================================
...
def build_identity_permutation_batch(
    batch_size: int,
    n: int,
    device: torch.device,
    dtype: torch.dtype,
) -> torch.Tensor:
    """Return a batched identity permutation tensor of shape (B, N, N)."""
    eye = torch.eye(n, device=device, dtype=dtype)
    return eye.unsqueeze(0).expand(batch_size, -1, -1).clone()


def _single_logical_canonical_order(A_single: torch.Tensor, m_single: torch.Tensor) -> List[int]:
    """Build a deterministic logical order from one sample.

    Strategy
    --------
    - restrict to active logical slots from m
    - choose the first root by highest total row score in A
    - perform a BFS-like expansion where neighbors are visited in descending
      interaction strength, then descending row score, then ascending index
    - if the active logical graph is disconnected, restart from the highest-score
      unvisited active node
    - append inactive padded slots at the end in identity order
    """
    if A_single.ndim != 2 or m_single.ndim != 1:
        raise ValueError('Expected A_single=(N,N) and m_single=(N,) for canonical ordering.')

    n = A_single.shape[0]
    active = [u for u in range(n) if float(m_single[u].item()) > 0.5]
    inactive = [u for u in range(n) if u not in active]

    if not active:
        return list(range(n))

    A_work = A_single.detach().cpu()
    row_score = A_work.sum(dim=-1)

    visited = set()
    order: List[int] = []

    def pick_next_root() -> int:
        remaining = [u for u in active if u not in visited]
        remaining.sort(key=lambda u: (-float(row_score[u].item()), int(u)))
        return int(remaining[0])

    while len(visited) < len(active):
        root = pick_next_root()
        queue = [root]
        ...
        while head < len(queue):
            ...
            nbrs = [v for v in active if v not in visited and v != u]
            nbrs.sort(
                key=lambda v: (
                    -float(A_work[u, v].item()),
                    -float(row_score[v].item()),
                    int(v),
                )
            )

            for v in nbrs:
                if float(A_work[u, v].item()) <= 0.0:
                    continue
                visited.add(v)
                order.append(v)
                queue.append(v)
...
def build_logical_canonical_teacher_permutation(
    A: torch.Tensor,
    m: torch.Tensor,
) -> torch.Tensor:
    """Construct a batched hard permutation teacher for R_L pretraining.

    Output convention matches the rest of the project: teacher[t, u] = 1 means
    original logical slot u is assigned to latent slot t.
    """
    A_b = _ensure_batched_square_matrix(A, 'A')
    m_b = _ensure_batched_vector(m, 'm')
    if A_b.shape[0] != m_b.shape[0]:
        raise ValueError('Batch mismatch between A and m in canonical teacher builder.')

    Bsz, n, _ = A_b.shape
    teacher = torch.zeros((Bsz, n, n), device=A_b.device, dtype=A_b.dtype)
    for b in range(Bsz):
        order = _single_logical_canonical_order(A_b[b], m_b[b])
        for t, u in enumerate(order):
            teacher[b, t, int(u)] = 1.0
    return teacher
...
class SoftPermutationReindexer(nn.Module):
    ...
    def __init__(
        ...
        # Trainer/eval may set this for clean R_H = I ablations.
        self.force_identity_hardware: bool = False
    ...
    def forward(
        ...
        use_identity_hardware = bool(freeze_hardware_reindex or self.force_identity_hardware)
        if use_identity_hardware:
            Bsz = A.shape[0] if A.ndim == 3 else 1
            dtype = A.dtype if torch.is_tensor(A) else Bmat.dtype
            device = A.device if torch.is_tensor(A) else Bmat.device
            R_H = build_identity_permutation_batch(Bsz, self.n, device=device, dtype=dtype)
        else:
...
class KMWModel(nn.Module):
    ...
    def forward(
        ...
        reidx = self.reindexer(
            ...
            D=D,
            tau_r=tau_r,
            freeze_hardware_reindex=freeze_hardware_reindex,
        )
...
__all__ = [
    ...
    "build_identity_permutation_batch",
