...
# - Updated LossConfig wiring and CLI flags to the v1.4.1 execution-surrogate
#   loss family.
# - Removed the old lambda_P / lambda_S / lambda_D / kappa_depth overrides.
# - Added CLI/config overrides for eps_surv and routing-step multipliers.
# =============================================================================
# =============================================================================
# UPDATE LOG (2026-03-30)
# - Added CLI/config support for C/D/E extensions:
#     * freeze_hardware_reindex ablation
#     * canonical-teacher pretraining
# - Added evaluation CLI support so identity-R_H inference can match the
#   training-time D ablation.
# =============================================================================
...
def build_loss_config_for_train(args) -> LossConfig:
    ...
    cfg.eps_surv = args.eps_surv
    cfg.route_step_2q_mult = args.route_step_2q_mult
    cfg.route_step_1q_mult = args.route_step_1q_mult
...
#     cfg.freeze_hardware_reindex = args.freeze_hardware_reindex

#     return cfg

# --- inside build_eval_config(args) ---
def build_eval_config(args) -> EvalConfig:
    """
    Build EvalConfig from defaults + JSON + CLI overrides.
    """
    ...
    # Hotfix: older parser snapshots may not expose this flag yet.
    cfg.freeze_hardware_reindex = getattr(args, "freeze_hardware_reindex", False)
...
def add_common_loss_override_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--eps-surv", type=float, default=1e-12, help="Clamp applied before negative-log survival transforms.")
    parser.add_argument("--route-step-2q-mult", type=float, default=3.0, help="Multiplier for native 2Q hazard in one routing step.")
    parser.add_argument("--route-step-1q-mult", type=float, default=2.0, help="Small 1Q-overhead multiplier used in one routing step.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # --- inside build_parser(), in the eval subparser section ---
    p_eval.add_argument(
        "--freeze-hardware-reindex",
        action="store_true",
        help="If set, force inference-time R_H = I and only use logical reindexing.",
    )
