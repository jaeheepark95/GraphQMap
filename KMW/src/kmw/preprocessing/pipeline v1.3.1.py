...
class BackendTensors:
    ...
    # Normalized tensors = values actually fed downstream.
    B: torch.Tensor
    c1: torch.Tensor
    c2: torch.Tensor
    D: torch.Tensor
...
def extract_backend_tensors(
    ...
    *,
    ...
    Output tensors:
    - B_raw: binary hardware adjacency
    - c1_raw: per-physical-qubit cost
    - c2_raw: per-edge cost
    - D_raw: graph shortest-path distance

    Then we build normalized versions B, c1, c2, D.
    """
    ...
    c1_raw = _extract_qubit_costs(backend, target, num_qubits)
    c2_raw = _extract_edge_costs(target, B_raw, num_qubits)
    D_raw = _all_pairs_shortest_paths(B_raw, unreachable_distance_fill)

    # Validate raw tensors before normalizing them.
    _validate_backend_raw_tensors(B_raw, c1_raw, c2_raw, D_raw, num_qubits)

    # Build normalized tensors.
    B = B_raw.copy()  # adjacency is already binary; keep it as-is
    c1 = _zscore(c1_raw, eps=eps)
    c2 = _normalize_edge_costs(c2_raw, B_raw, eps=eps)
    D = _max_normalize(D_raw, eps=eps)

    # Validate normalized tensors too.
...
def featurize_circuit(
    ...
    *,
    ...
    # We separately count 1-qubit activity and 2-qubit participation.
