...
# Note: routing functions seem completely omitted from evaluate1.py
...
def print_console_summary(summary: Dict[str, Any]) -> None:
    ...
    print("\nRouting summary:")
    print(f"  implemented = {routing.get('implemented', False)}")

    if routing.get("implemented", False):
        print(f"  backend_name = {routing.get('backend_name')}")
        print(f"  routing_method = {routing.get('routing_method')}")
        print(f"  optimization_level = {routing.get('optimization_level')}")
        print(f"  include_readout_in_pst = {routing.get('include_readout_in_pst')}")
        print(f"  successful_routing_rows = {routing.get('successful_routing_rows')}")
        print(f"  routing_failure_rows = {routing.get('routing_failure_rows')}")
        status_breakdown = routing.get("routing_status_breakdown", {})
        if status_breakdown:
            print("  routing_status_breakdown:")
            for status, count in sorted(status_breakdown.items()):
                print(f"    {status}: {count}")
    else:
        print(
            f"  note = {routing.get('note', 'Routing-based downstream metrics are disabled for this run.') }"
        )
