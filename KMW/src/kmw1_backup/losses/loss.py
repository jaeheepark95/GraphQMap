...
from kmw1.models.model import decode_canonical_to_native_logits, sinkhorn_assignment
...
def _compute_route_cost_matrix(
    *,
    ...
    """
    Reliability-weighted routing hazard used by v1.4.1.

    c_step(i,j) = 3 q_2q(i,j) + 0.5 * 2 * (q_1q(i) + q_1q(j))
                = 3 q_2q(i,j) + q_1q(i) + q_1q(j)
    """
    ...
    c_step = config.route_step_2q_mult * q2q + 0.5 * config.route_step_1q_mult * (
        q1q.unsqueeze(-1) + q1q.unsqueeze(-2)
    )
    c_step = c_step * Bmat
...
def compute_task_loss_from_assignment(
    *,
    ...
    """
    Authoritative task loss for this branch:
    decode to native frame first, then compute v1.4.1 in native hardware order.
    """
    if config is None:
        config = LossConfig()
    ...
    C_route = _compute_route_cost_matrix(
        Bmat=Bmat,
        D_raw=D_raw,
        e1q=e1q,
        e2q=e2q,
...
def compute_task_loss_from_logits(
    *,
    ...
    P_map = sinkhorn_assignment(S_logits=S_nat, tau=config.sinkhorn_tau, num_iters=config.sinkhorn_iters)
