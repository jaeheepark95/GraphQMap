...
1. explicit implementation decisions locked for **Canonical Design Plan v1.41**,
...
The canonical-hardware v1.41 path is now:
...
KMW/
├── readme.md
├── Canonical Design Plan v1.41.md
│
├── configs/
│   ├── base.yaml
│   ├── smoke_mqt.yaml
...
- `src/kmw1/data/dataset.py` — manifest-driven dataset loading, split selection, circuit-file lookup, and per-sample return contract
...
- `src/kmw1/losses/loss.py` — v1.4.1 execution-surrogate loss in the **native frame after deterministic decode**, plus loss-side validation helpers
- `src/kmw1/training/trainer.py` — optimizer/scheduler setup, train/validation loops, checkpointing, logging, fail-fast checks, and Option A training orchestration
- `src/kmw1/evaluation/evaluate.py` — inference path, deterministic decode, native-frame assignment, proxy metrics, and **mandatory** routed evaluation outputs (`real_pst_gate_readout`, SWAP/depth/compile-time metrics), plus CSV/JSON summary generation
...
# Canonical Design Plan v1.41 — Implementation Specification
...
- [12. Task Loss (v1.4.1 in canonical frame)](#12-task-loss-v141-in-canonical-frame)
...
During training, apply Sinkhorn to the **decoded native-frame logits**:

\[
P_{\mathrm{nat}} = \mathrm{Sinkhorn}(S_{\mathrm{nat}} / 	au)
\]
...
If Hungarian assigns logical qubit `u` to native hardware ID `i`, then that native ID is the final reported placement.
...
5. compute `P_nat` from `S_nat`
6. compute native-frame-after-decode v1.4.1 loss
...
7. native-frame-after-decode v1.4.1 loss
8. trainer
9. evaluation
...
The canonical-hardware v1.41 branch keeps the streamlined v1.4 mapper architecture — circuit matrix as the only spatial U-Net input, hardware-only tokens as cross-attention conditioning, Sinkhorn for training, Hungarian for inference, and the v1.4.1 execution-surrogate loss family — but removes the learned reindexer entirely. Instead, the hardware is canonicalized once using a deterministic permutation computed from the backend topology and costs, while the circuit remains in original logical order. The mapper therefore predicts into canonical hardware slots, those logits are deterministically decoded back to native hardware order, assignment and task loss are evaluated in the native frame after decode, 

and the resulting native-ID mapping flows directly into downstream routed evaluation. This branch exists to isolate whether learned reindexing was the dominant cause of the performance drop observed in the learned-reindexer v1.4 pipeline.
