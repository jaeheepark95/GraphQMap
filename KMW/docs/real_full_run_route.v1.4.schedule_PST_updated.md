...
- Removed the duplicate training block.
- Fixed the evaluation path so that:
    (1) CKPT and RUN_DIR must match the actual checkpoint folder
    (2) loss_config_v141.json is recreated if missing before eval
===============================================================================
-->
...
#    set RUN_DIR to the exact folder containing latest.pt
#    and set CKPT="$RUN_DIR/latest.pt"
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4a) ensure eval loss config exists in the same checkpoint folder
# -----------------------------------------------------------------------------
# This is required when reusing an existing checkpoint folder that has latest.pt
# but does not yet contain loss_config_v141.json.
# -----------------------------------------------------------------------------

test -f "$RUN_DIR/loss_config_v141.json" || cat > "$RUN_DIR/loss_config_v141.json" <<'JSON'
{
  "tau_m": 0.10,
  "sinkhorn_iters": 20,
...
test -f "$RUN_DIR/loss_config_v141.json" && echo "Loss config OK: $RUN_DIR/loss_config_v141.json"
