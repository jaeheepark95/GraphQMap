...
# CURRENT KMW1 IMPLEMENTATION VERSION (v1.41-style package / clean canonical-only CLI)
# =============================================================================
#
# Purpose
# -------
# This run sheet is the current canonical-hardware run sheet for the kmw1 branch.
#
# - training side:
#     QUEKO + MQT Bench
...
#
# Current kmw1 branch assumptions
# -------------------------------
# - package namespace: src/kmw1
# - CLI entrypoint: python -m kmw1.cli.main
# - no learned reindexer
# - no tau_r / Pass-A / Pass-B / alpha_loc / beta_cons
# - deterministic hardware canonicalization
# - native-after-decode loss path
# - routed eval settings passed through --eval-config JSON
#
# Locked/default settings used here
# ---------------------------------
...
# ------------------------------------------
# - On a fresh first run after code changes, clear stale caches once and use
#   --force-recompute.
# - If you want best.pt, training must include --val-manifest.
# - Current automatic best-checkpoint selection should target:
#       "val_mean_L_task_hard"
#   not the older "val_L_task".
# - For reliability, train + eval separately is safer than relying on full-run.
#
# =============================================================================
...
# -----------------------------------------------------------------------------
# 0a) one-time cache reset for fresh canonical branch runs
# -----------------------------------------------------------------------------
# Do this once after changing kmw1 preprocessing/cache schema.
# You can comment these lines out after the cache is rebuilt cleanly.
# -----------------------------------------------------------------------------

rm -rf data/cache/circuits/*
rm -rf data/cache/backends/*
...
#    set RUN_DIR to the folder containing the kmw1 run outputs
#    and set CKPT to the exact checkpoint you want to use
#
# Recommended checkpoint choices:
# - if you trained WITHOUT --val-manifest:
#       CKPT="$RUN_DIR/checkpoints/epoch_060.pt"
# - if you trained WITH --val-manifest:
#       CKPT="$RUN_DIR/checkpoints/best.pt"
# -----------------------------------------------------------------------------

RUN_DIR=artifacts/fullrun_queko_mqt_kmw1_current_seed20260331
mkdir -p "$RUN_DIR"
...
# -----------------------------------------------------------------------------
# 3) OPTIONAL: training run with current canonical kmw1 branch
# -----------------------------------------------------------------------------
# Run this section only if you need a fresh checkpoint.
# If you already have a trained checkpoint, skip to section 4.
# -----------------------------------------------------------------------------
...
cat > "$RUN_DIR/trainer_config_kmw1.json" <<'JSON'
{
  ...
  "save_best_on": "val_mean_L_task_hard"
}
JSON
...
# If you want best.pt instead of epoch_060.pt, use:
#   --val-manifest "$QASMBENCH_VAL"
# in the training command above, then set:
#   CKPT="$RUN_DIR/checkpoints/best.pt"
...
# -----------------------------------------------------------------------------
# 4a) ensure loss config and base eval config exist
# -----------------------------------------------------------------------------
# In kmw1, routed-eval knobs live inside --eval-config JSON.
# -----------------------------------------------------------------------------
...
cat > "$RUN_DIR/eval_config_pst.json" <<'JSON'
{
  "print_console_summary": true,
  "fail_fast": false,
...
python -m kmw1.cli.main eval \
  --project-root "$PWD" \
  --manifest data/manifests/full/recipes/heldout_qasmbench/test.jsonl \
  --checkpoint "$CKPT" \
  --backend-name fake_toronto_v2 \
  --device cuda \
  --seed 20260331 \
  ...
  --sinkhorn-iters 30 \
  --loss-config "$RUN_DIR/loss_config_kmw1.json" \
  --eval-config "$RUN_DIR/eval_config_pst.json" \
  --per-circuit-csv "$RUN_DIR/qasmbench_test_per_circuit_pst.csv" \
  --summary-json "$RUN_DIR/qasmbench_test_summary_pst.json" \
...
# - failures/failure_eXXX_sXXXXXX.json        # only if a fail-fast batch abort occurs
...
#
# Current per-circuit eval rows should include fields like:
# - L_task_hard
# - L_native_hard
# - L_route_hard
# - S_proxy_exec_hard
