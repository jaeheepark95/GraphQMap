...
#NASSC method is based on SABRE method
...
class NASSCSwap(TransformationPass):
    
    def __init__(self, coupling_map, heuristic="basic", seed=None, fake_run=False, enable_factor_block=False, enable_factor_commute_0=False, enable_factor_commute_1=False, factor_block=0, factor_commute_0=0, factor_commute_1=0, decomposer2q=None, approximation_degree=None):
    ...
    def run(self, dag):
        ...
        while front_layer:
                ...
                if len(predecessor_block)>0:
                    ...
                    if(self.enable_factor_commute_0 or self.enable_factor_commute_1):
                        ...
                        if (self.enable_factor_commute_0 and swap_commutation[swap_qubits][0]):
                            ...
                            swap_scores[swap_qubits] -= 2*self.factor_commute_1
                swap_scores[swap_qubits] = self._score_heuristic_recalculate(
                    swap_scores[swap_qubits], self.heuristic, front_layer, extended_set, trial_layout, swap_qubits
                )    
            ...
            swap_node = DAGOpNode(op=SwapGate(), qargs=list(best_swap))
        ...
        if swap_commutation:
            ...
            if swap_commutation[3]:
                ...
                if not swap_commutation[2].cargs:
                    swap_commutation[2].cargs = tuple()
            ...
            if swap_commutation[4]:
                new_node = mapped_dag.apply_operation_back(new_node.op, [new_node.qargs[1],new_node.qargs[0]], new_node.cargs, check=True)
                ...
                new_node = mapped_dag.apply_operation_back(new_node.op, new_node.qargs, new_node.cargs, check=True)
...
def _transform_gate_for_layout(op_node, layout, device_qreg):
    ...
    mapped_qargs = []
    for qarg in premap_qargs:
        mapped_qargs.append(device_qreg[layout[qarg]])
